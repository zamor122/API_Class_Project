...
def searchForSomething():
    ...
    try:
        ...
        userAnswer = raw_input(
            'Enter a type of place from the list above exactly as it appears: ')
    ...
    ########################### ERROR CATCHING ###################################################
        ...
        print 'ERROR: Something happened! Please restart the program!'
    ...
    try:
        ...
        while True:
            try:
                    ...
                    '\nEnter the address you want to end your travels.\n ')
                if end.__contains__(','):
                    endLocation = end.lower().replace(" ", "")
                    break
                else:
                    raise ValueError
            except ValueError:
                print 'ERROR: Please enter a valid address or the words "current location"'
        searchForSomethingByAddress(startLocation,endLocation)
    except KeyError:
...
#begin function to search for place by address
def searchForSomethingByAddress(start, finish):
    distanceRequest = "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=" + start + "&destinations=" + finish + "&key=AIzaSyCTr-K7HHEipMPY6UdPdVH7fd5TaBH3gLM"
    # print distanceRequest
    # store request
    timeResult = urllib.urlopen(distanceRequest)
    # distanceRequest
    ...
    niceList = listOfTimes
    print'\nRESULTS'
    getTimeTravel(start, finish)
    print 'It will take you ' + niceList + ' to get to there!\nEND RESULTS'